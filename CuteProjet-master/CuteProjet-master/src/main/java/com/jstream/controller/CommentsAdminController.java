...
public class CommentsAdminController implements Initializable {
    ...
    private Callback<TableColumn<Comment, Void>, TableCell<Comment, Void>> buildActionFactory() {
        return param -> new TableCell<>() {
            ...
            private final Button btnFlag = new Button();
            ...
            protected void updateItem(Void item, boolean empty) {
                ...
                if (empty || getTableRow() == null || getTableRow().getItem() == null) {
                    ...
                    Comment c = getTableRow().getItem();
                    if (c.isFlagged()) {
                        btnFlag.setText("✅ Valider");
                        btnFlag.setStyle("-fx-background-color: #4CAF50; -fx-text-fill: white; " +
                                "-fx-cursor: hand; -fx-background-radius: 5; -fx-font-size: 11px;");
                    } else {
                        btnFlag.setText("⚠ Signaler");
    ...
    private void loadData() {
        try {
            List<Comment> updatedComments = commentDAO.findAll();
            allComments.setAll(updatedComments);
