...
public class DashboardAdminController implements Initializable {
    ...
    private StackPane createCard(String title, String type, String coverUrl) {
    ...
    private void handleSaveCategory(ActionEvent event) {
        try {
            if (categoryNameField == null || statusLabel == null) return;
    ...
    private void handleAddFilm(ActionEvent event) {
        try {
            ...
            try {
                if (yearText != null && !yearText.isEmpty()) Integer.parseInt(yearText);
            ...
            if (filmToEdit == null) {
                Film nouveauFilm = new Film(0, title, synopsis, cast, yearText, videoUrl, posterUrl, categoryId);
                AdminService adminService = new AdminService();
            ...
            e.printStackTrace();
            statusLabel.setText("Erreur serveur lors de l'opération sur le film.");
            statusLabel.setStyle("-fx-text-fill: #ff4444;");
        }
    }
    ...
    private void handleAddEpisode(ActionEvent event) {
        try {
            if (episodeTitleField == null || episodeDescriptionField == null || statusLabel == null) return;
            ...
            if (statusLabel != null) {
                statusLabel.setText("❌ Le numéro et la durée doivent être des chiffres valides.");
                statusLabel.setStyle("-fx-text-fill: red;");
            }
        } catch (Exception e) {
            if (statusLabel != null) {
    ...
    private void handleAddSeries(ActionEvent event) {
        try {
            if (titleField == null || synopsisField == null || statusLabel == null) return;
    ...
    private void refreshMetrics() {
        try {
            ...
            if (totalViewsLabel != null) totalViewsLabel.setText(filmCount + seriesCount + " contenus");
            if (totalSubscribersLabel != null) totalSubscribersLabel.setText(String.valueOf(userCount));
            if (newSubscribersLabel != null) newSubscribersLabel.setText(String.valueOf(userCount));
            if (pendingLabel != null) pendingLabel.setText(String.valueOf(filmCount));
            if (canceledLabel != null) canceledLabel.setText(String.valueOf(seriesCount));
        } catch (Exception e) {
    ...
    private void refreshBarChart() {
        ...
        String filter = (barChartFilter != null && barChartFilter.getValue() != null)
                ? barChartFilter.getValue() : "12 derniers mois";
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            Tooltip tip = new Tooltip(labels[i] + " : " + values[i] + "k vues");
    ...
    private void refreshLineChart() {
        ...
        String filter = (lineChartFilter != null && lineChartFilter.getValue() != null)
                ? lineChartFilter.getValue() : "4 derniers mois";
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            Tooltip tip = new Tooltip(labels[i] + " : +" + values[i] + " abonnés");
            Tooltip.install(bar, tip);
    ...
    private void loadTopContent() {
        ...
        try {
            ...
            for (Film f : filmDAO.findAll())
                allTopContent.add(new TopContent(f.getTitle(),
                        (int)(Math.random() * 900 + 100) + "k", "Actif", "Film"));

            for (Series s : seriesDAO.findAll())
                allTopContent.add(new TopContent(s.getTitle(),
                        (int)(Math.random() * 700 + 50) + "k", "Actif", "Série"));
    ...
    @FXML
    private void onBarChartFilterChanged(ActionEvent event) { refreshBarChart(); }

    @FXML
    private void onLineChartFilterChanged(ActionEvent event) { refreshLineChart(); }
